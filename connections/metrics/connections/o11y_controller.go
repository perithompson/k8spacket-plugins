...
import (
	...
	"github.com/perithompson/k8spacket-plugins/connections/metrics/connections/stats"
)
...
func connectionsFieldsHandler(w http.ResponseWriter, r *http.Request) {
...
func connectionsDataHandler(w http.ResponseWriter, r *http.Request) {
	...
	for _, ip := range k8spacketIps {
		resp, err := http.Get(fmt.Sprintf("http://%s:%s/connections/connections?%s", ip, os.Getenv("K8S_PACKET_TCP_LISTENER_PORT"), r.URL.Query().Encode()))
...
func buildApiResponse(w http.ResponseWriter, connectionItems map[string]model.ConnectionItem, connectionEndpoints map[string]model.ConnectionEndpoint, query url.Values) {
	...
	response := model.Connections{nodeArray, edgeArray}
